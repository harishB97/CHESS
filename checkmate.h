...
{
	if(!(underAttack(location[33].first,location[33].second,300,side)))		// 300 --> NULL move - moves the king nowhere
	{
		return false;
	}
...
else if(side == true) //white
{
	...
	if(!(underAttack(location[13].first,location[13].second,0,side)))
	{
		return false;
	}
